            ...
            <div class="col-lg-6">
                ...
                <div class="d-flex justify-content-center mt-2">
                    <div class="d-flex justify-content-center">
                        <div class="mr-2">
                            <button type="button" class="btn btn-warning btn-lg btn-block" id="btn-downvote">
                                <i class="fa-solid fa-thumbs-down fa-2x fa-flip-horizontal"></i>
                                <p class="m-0 p-0">{{$produk->produkFeedback()->where('downvote', 1)->count()}}</p>
                            </button>
                        </div>
                        <div class="ml-2">
                            <button type="button" class="btn btn-warning btn-lg btn-block" id="btn-upvote">
                                <i class="fa-solid fa-thumbs-up fa-2x"></i>
                                <p class="m-0 p-0">{{$produk->produkFeedback()->where('upvote', 1)->count()}}</p>
                            </button>
                        </div>
                    </div>            
                </div>
            ...
            <div class="col-lg-5 offset-lg-1">
                <form action="/cart" method="POST">
                    ...
                    <div class="s_product_text">
                        ...
                        <ul class="list">
                            <li><a class="active"
                                    href="/katalog/category/{{$produk->kategori->id}}"><span>Category</span> :
                                    {{$produk->kategori->nama}}</a></li>
...
@section('script')
<script>
    $('button[id^="btn-upvote"]').on('click', function() 
    {
        $.ajax({
            url: "{{ route('produk-feedbacks.store') }}",
            type: 'POST',
            data: {
                _token: '{{ csrf_token() }}',
    ...
    $('button[id^="btn-downvote"]').on('click', function() 
    {
        $.ajax({
            url: "{{ route('produk-feedbacks.store') }}",
            type: 'POST',
